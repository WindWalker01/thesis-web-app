...
// ╔═══════════════════════════════════════════════════════════════════════════╗
// ║                          MOCK DATA                                        ║
// ║  ⚠️  Everything inside this block is placeholder data.                   ║
// ║  When connecting to the real API, replace each constant with your         ║
// ║  actual fetch/query result. Each constant is labelled with the            ║
// ║  shape the backend should return.                                         ║
// ╚═══════════════════════════════════════════════════════════════════════════╝

// ── Stat Cards ───────────────────────────────────────────────────────────────
// TODO: Replace values with real API response
// Expected shape: { totalArtworks: number, verified: number, totalUpvotes: number, openIssues: number }
const STATS_MOCK = {
  totalArtworks: 48,
  verified: 41,
  totalUpvotes: "3.4k",
...
// TODO: Replace each key's array with real API data for that time range.
//
// Expected shape per entry: { label: string; votes: number }
//   • "label" — the x-axis tick (hour, day name, month, year, etc.)
//   • "votes"  — total upvotes in that bucket
//
// How to wire to API:
//   When the user selects a range, call e.g. GET /api/upvotes?range=7d
//   and replace the matching key below with the response array.
//
// NOTE on spikes:
//   • "24h"  — spike at 8 PM (viral share on social media)
//   • "7d"   — spike on Friday (weekly post peaked)
//   • "1m"   — spike on Mar 21 (Ethereal Bloom dropped)
//   • "1y"   — spike in March overall
//   • "max"  — 2024 Q1 ramp-up, Mar 2024 massive viral spike

const CHART_DATA_MOCK: Record<string, { label: string; votes: number }[]> = {
  // 24h — hourly buckets, 0:00 → 23:00 (every 2 hours shown for readability)
  "24h": [
    { label: "12a", votes: 8 },
    { label: "2a",  votes: 4 },
    { label: "4a",  votes: 2 },
    ...
    { label: "4p",  votes: 78 },
    { label: "6p",  votes: 92 },
    { label: "8p",  votes: 187 }, // 🔴 spike — viral social share
    { label: "10p", votes: 74 },
  ],
  ...
  "7d": [
    ...
    { label: "Wed", votes: 201 },
    { label: "Thu", votes: 158 },
    { label: "Fri", votes: 312 }, // 🔴 spike — weekly post peak
    { label: "Sat", votes: 278 },
    { label: "Sun", votes: 193 },
  ],

  // 1m — daily buckets for the past 30 days (sampled every ~3 days for clarity)
  "1m": [
    { label: "Mar 1",  votes: 95 },
    { label: "Mar 4",  votes: 118 },
    { label: "Mar 7",  votes: 143 },
    { label: "Mar 10", votes: 167 },
    { label: "Mar 13", votes: 204 },
    { label: "Mar 16", votes: 239 },
    { label: "Mar 19", votes: 291 },
    { label: "Mar 21", votes: 748 }, // 🔴 spike — "Ethereal Bloom" uploaded
    { label: "Mar 24", votes: 334 },
    { label: "Mar 27", votes: 278 },
    { label: "Mar 30", votes: 241 },
    { label: "Apr 1",  votes: 198 },
  ],
  ...
  "1y": [
    ...
    { label: "Jan",  votes: 519 },
    { label: "Feb",  votes: 391 },
    { label: "Mar",  votes: 1480 }, // 🔴 spike — biggest viral month
    { label: "Apr",  votes: 641 },
  ],

  // max — yearly buckets (all time)
  "max": [
    { label: "2022", votes: 340 },
    { label: "2023", votes: 1820 },
    { label: "2024", votes: 5240 }, // 🔴 spike — platform growth + viral posts
    { label: "2025", votes: 3402 },
  ],
};

// ── Top Posts ─────────────────────────────────────────────────────────────────
// TODO: Replace with real posts sorted by upvote count (DESC), limit 4
// Expected shape: { id: string; title: string; category: string; upvotes: number; views: number; trend: "up" | "down"; color: string; timeAgo: string }[]
const TOP_POSTS_MOCK: {
  ...
  { id: "1", title: "Ethereal Bloom",  category: "Digital Painting", upvotes: 1240, views: 4800, trend: "up",   color: "from-blue-500 to-purple-600",  timeAgo: "2 days ago" },
  { id: "2", title: "Void Geometry",   category: "AI Art",           upvotes: 987,  views: 3100, trend: "up",   color: "from-violet-500 to-indigo-600", timeAgo: "5 days ago" },
  { id: "3", title: "Neon Samurai",    category: "Illustration",     upvotes: 654,  views: 2300, trend: "down", color: "from-orange-500 to-red-600",    timeAgo: "1 week ago" },
  { id: "4", title: "Fractured Light", category: "Photography",      upvotes: 401,  views: 1500, trend: "up",   color: "from-cyan-500 to-blue-500",     timeAgo: "2 weeks ago" },
];

// ── Engagement Snapshot ───────────────────────────────────────────────────────
// TODO: Replace with real aggregated engagement stats
// Expected shape: { totalUpvotes: string; totalViews: string; avgUpvotesPerPost: string; mostUpvotedTitle: string }
const ENGAGEMENT_MOCK = {
  totalUpvotes: "3,402",
  totalViews: "11.7k",
  avgUpvotesPerPost: "70.9",
  mostUpvotedTitle: "Ethereal Bloom",
};

// ── Recent Activity ───────────────────────────────────────────────────────────
// TODO: Replace with real activity log (last 4 events)
// Expected shape: { icon: LucideIcon; color: string; bg: string; text: string; time: string }[]
const RECENT_ACTIVITY_MOCK = [
  { icon: ArrowUp,       color: "text-orange-400", bg: "bg-orange-500/10", text: "Ethereal Bloom received 42 new upvotes",    time: "2 hours ago" },
  { icon: ShieldCheck,   color: "text-green-400",  bg: "bg-green-500/10",  text: "Ownership verified for Void Geometry",      time: "Yesterday" },
  { icon: AlertTriangle, color: "text-red-400",    bg: "bg-red-500/10",    text: "Plagiarism flag raised on Neon Samurai",    time: "2 days ago" },
  { icon: Upload,        color: "text-blue-400",   bg: "bg-blue-500/10",   text: "Fractured Light uploaded successfully",     time: "Apr 3, 2025" },
];
...
type TimeRange = "24h" | "7d" | "1m" | "1y" | "max";
...
// stat cards config (values pulled from STATS_MOCK)
const statCards = [
  { label: "Your Artworks", value: STATS_MOCK.totalArtworks, hint: "Total pieces uploaded",      icon: ImageIcon,    color: "text-blue-400",   bg: "bg-blue-500/10",   border: "border-blue-500/20"   },
  { label: "Verified",       value: STATS_MOCK.verified,      hint: "Blockchain confirmed",        icon: ShieldCheck,  color: "text-green-400",  bg: "bg-green-500/10",  border: "border-green-500/20"  },
  { label: "Total Upvotes",  value: STATS_MOCK.totalUpvotes,  hint: "Across all gallery posts",    icon: ArrowUp,      color: "text-orange-400", bg: "bg-orange-500/10", border: "border-orange-500/20" },
  { label: "Open Issues",    value: STATS_MOCK.openIssues,    hint: "Flagged or removed",          icon: AlertTriangle, color: "text-red-400",   bg: "bg-red-500/10",    border: "border-red-500/20"    },
];
...
export default function DashboardPage() {
  ...
  // Derived chart values — auto-update when range changes
  const chartData = CHART_DATA_MOCK[activeRange];
  const chartMax  = Math.max(...chartData.map((d) => d.votes));
  const chartTotal = chartData.reduce((s, d) => s + d.votes, 0);
  const chartPeak  = chartData.reduce((a, b) => (a.votes > b.votes ? a : b));
        ...
        <div className="container mx-auto px-4 py-12 md:px-6 md:py-16">
          <div className="grid gap-8 lg:grid-cols-[1.3fr_0.7fr] lg:items-end">
            ...
            <div className="grid grid-cols-3 gap-2.5">
              <HeroStatCard label="Artworks"   value="48 total"   />
              <HeroStatCard label="Verified"   value="41 on-chain" />
              <HeroStatCard label="Upvotes"    value="3.4k total" />
              <HeroStatCard label="pHash Check" value="Active"    />
              <HeroStatCard label="Crypto Hash" value="Integrity" />
              <HeroStatCard label="RA 8293"    value="Protected"  />
            </div>
          </div>
        </div>
      </section>
            ...
            */}
                  ...
                  <div className="flex items-center gap-1 bg-muted/60 rounded-xl p-1 border border-border">
                    {TIME_RANGE_OPTIONS.map(({ key, label }) => (
                      <button
                        ...
                        className={`relative px-3 py-1.5 rounded-lg text-[11px] font-bold transition-all duration-200 ${
                          activeRange === key
                            ? "text-foreground"
                  ...
                  {chartData.map((d) => (
                    <div key={d.label} className="flex-1 text-center">
                      <span className={`text-[9px] font-semibold leading-none ${
                        d.label === chartPeak.label ? "text-orange-400" : "text-muted-foreground"
                      }`}>
                        {d.label}
                      </span>
                    </div>
            ...
            >
              ...
              <div className="px-5 py-4 space-y-4">
                {[
                  { label: "Total Upvotes",      value: ENGAGEMENT_MOCK.totalUpvotes,      icon: ArrowUp,    color: "text-orange-400", bg: "bg-orange-500/10" },
                  { label: "Total Views",         value: ENGAGEMENT_MOCK.totalViews,        icon: Eye,        color: "text-blue-400",   bg: "bg-blue-500/10"   },
                  { label: "Avg. Upvotes / Post", value: ENGAGEMENT_MOCK.avgUpvotesPerPost, icon: Heart,      color: "text-pink-400",   bg: "bg-pink-500/10"   },
                  { label: "Most Upvoted Post",   value: ENGAGEMENT_MOCK.mostUpvotedTitle,  icon: TrendingUp, color: "text-green-400",  bg: "bg-green-500/10", small: true },
                ].map((item) => {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            >
              ...
              <div className="px-4 py-3 space-y-1">
                {RECENT_ACTIVITY_MOCK.map((item, i) => {
                  const Icon = item.icon;
                  return (
                    <motion.div
                      key={i}
          ...
          >
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-start sm:items-center gap-5 justify-between">
              <div className="flex items-center gap-4">
                ...
                <div>
                  {/* TODO: Replace with STATS_MOCK.verified when wiring real data */}
                  <p className="text-white font-black text-base leading-tight">{STATS_MOCK.verified} Artworks Protected</p>
                  <p className="text-slate-400 text-xs mt-0.5">Secured under R.A. 8293 · Philippine IP Law</p>
                </div>
              </div>
