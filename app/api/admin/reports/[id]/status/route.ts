...
import { updateReportStatusSchema } from "@/features/reports/schemas/report-schemas";
...
export async function PATCH(
  ...
  try {
    ...
    const result = await service.updateReportStatusWithAudit(supabase, {
      reportId: id,
      newStatus,
      adminId: user.id,
      notes,
    ...
    const statusCode = message.includes("not found")
      ? 404
      : message.includes("Cannot transition") || message.includes("already in this status")
        ? 400
        : 500;
