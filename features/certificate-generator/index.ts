...
/**
 * Resolves the absolute base URL used to build the public certificate
 * verification link. Prefers the configured site URL and falls back to the
 * current origin (useful during local development).
 */
...
/**
 * Generates a PNG data-URL QR code pointing to the public verification page.
 * Returns null if QR generation fails so it never blocks certificate download.
 */
...
export async function generateArtworkCertificatePdf(
  ...
  // Public verification link encoded into the QR code
  const baseUrl = resolveBaseUrl();
  const verifyUrl = `${baseUrl}/certificate/verify/${safeValue(artwork.id)}`;
  const qrDataUrl = await generateVerificationQr(verifyUrl);
  ...
  doc.text(
    "ARTFORGELAB  ·  INTELLECTUAL PROPERTY RIGHTS MANAGEMENT SYSTEM",
    34,
    19,
  );
  ...
  doc.setTextColor(100, 116, 139);
  doc.text(
    `Ref: #${safeValue(artwork.id).slice(0, 12).toUpperCase()}`,
    W - 12,
    62,
    { align: "right" },
  );
  ...
  // ── Verification QR ──────────────────────────────────────────
  // White card in the header holding the scannable verification QR code.
  if (qrDataUrl) {
    const qrSize = 22;
    const qrPad = 2;
    const qrCardSize = qrSize + qrPad * 2;
    const qrX = W - 12 - qrCardSize;
    const qrY = 6;

    doc.setFillColor(255, 255, 255);
    roundedRect(doc, qrX, qrY, qrCardSize, qrCardSize, 2, "F");

    try {
      doc.addImage(qrDataUrl, "PNG", qrX + qrPad, qrY + qrPad, qrSize, qrSize);
    } catch {
      // Ignore image failures — the certificate is still valid without the QR.
    }

    // Clickable hotspot over the QR
    doc.link(qrX, qrY, qrCardSize, qrCardSize, { url: verifyUrl });

    doc.setFont("helvetica", "bold");
    doc.setFontSize(5.5);
    doc.setTextColor(148, 163, 184);
    doc.text("SCAN TO VERIFY", qrX + qrCardSize / 2, qrY + qrCardSize + 3, {
      align: "center",
    });
  }
  ...
  if (artwork.img) {
    ...
    doc.setFontSize(7);
    doc.setTextColor(148, 163, 184);
    doc.text("No Image", imgBoxX + imgBoxSize / 2, imgBoxY + imgBoxSize / 2, {
      align: "center",
    });
  }
  ...
  const hashRows: Array<[string, string]> = [
    ...
    [
      "Perceptual Hash",
      shortenMiddle(safeValue(artwork.perceptualHash), 12, 8),
    ],
  ...
  const declH = Math.max(
    30,
    declHeaderOffset + declTextHeight + declBottomPadding,
  );
  ...
  doc.setFontSize(6.5);
  doc.setTextColor(100, 116, 139);
  doc.text(`Generated on ${new Date().toLocaleString()}`, W - 14, H - 6.5, {
    align: "right",
  });
