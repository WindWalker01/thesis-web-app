...
raw_similarity: 22.4,  raw_similarity_legacy: 64.1,  calibrated_confidence: 99.2,  transform_consistency: 1.0,  transform_agreements: 4,  block_agreements: 3,  content_blocks_used: 5,  low_content_warning: false,
};
/** v3 match with transform evidence absent (likely a crop/zoom copy). */const v3CropMatch: MatchMetrics = {
raw_similarity: 34.25,  raw_similarity_legacy: 60.75,  calibrated_confidence: 100,  transform_consistency: 0,  transform_agreements: 0,  block_agreements: 2,  content_blocks_used: 5,  low_content_warning: false,  transform_evidence_status: "absent",  block_evidence_status: "checked",  best_scale_pair: ["0.625", "0.75"],  dominant_transform: null,
};
/** v3 match with rotation evidence. */const v3RotationMatch: MatchMetrics = {
raw_similarity: 19.57,  raw_similarity_legacy: 55.2,  calibrated_confidence: 95,  transform_consistency: 1.0,  transform_agreements: 4,  block_agreements: 3,  content_blocks_used: 5,  low_content_warning: false,  transform_evidence_status: "checked",  block_evidence_status: "checked",  best_scale_pair: ["1.0", "1.0"],  dominant_transform: "180",
};
...
expect(isNoEvidenceMatch({
...v2CleanNegative, raw_similarity: 0.1
})).toBe(false);
expect(isNoEvidenceMatch({
...v2CleanNegative, transform_consistency: 1.0
})).toBe(false);
expect(isNoEvidenceMatch({
...v2CleanNegative, block_agreements: 1
})).toBe(false);
expect(isNoEvidenceMatch({
...v2CleanNegative, transform_agreements: 1
})).toBe(false);
});
...
expect(getEvidenceSummary(v2Positive)).toBe(      "3 of 5 regions matched across 4 of 6 transform variants"    );
});
...
expect(getEvidenceDetail(v3RotationMatch)).toBe(      "3 of 5 regions matched, consistent under 180° rotation"    );
});
it("notes transform-evidence-absent status for crop matches", () => {
expect(getEvidenceDetail(v3CropMatch)).toBe(      "2 of 5 regions matched — matched on image content only, no rotation/flip detected (likely a crop)"    );
});
it("falls back to basic summary for v2 matches without v3 fields", () => {
expect(getEvidenceDetail(v2Positive)).toBe(      "3 of 5 regions matched across 4 of 6 transform variants"    );
...
expect(isLowContent({
...v2CleanNegative, low_content_warning: true
})).toBe(true);
...
expect(getTransformEvidenceStatus({
...v2Positive, transform_evidence_status: "absent"
})).toBe("absent");
expect(getTransformEvidenceStatus({
...v2Positive, transform_evidence_status: "checked"
})).toBe("checked");
});
...
it("returns the status for v3 matches", () => {
expect(getBlockEvidenceStatus({
...v2Positive, block_evidence_status: "absent"
})).toBe("absent");
expect(getBlockEvidenceStatus({
...v2Positive, block_evidence_status: "checked"
})).toBe("checked");
});
it("returns null for legacy matches without the field", () => {
...
expect(getBestScalePair({
...v2Positive, best_scale_pair: ["0.625", "0.75"]
})).toEqual(["0.625", "0.75"]);
});
it("returns null for legacy matches without the field", () => {
...
/** The shared mapping must work identically on all four match-object shapes. */describe("shared mapping across response shapes", () => {
const dbMatch: SearchMatch = {
type: "database",    source: "Registered Artwork",    url: "3213a9dc-ff10-40f2-8a1e-17017d1b40cb",    similarity: 0,    ...v2CleanNegative,
};
const otherMatch: OtherSearchMatch = {
source: "Database",    link: "https://example.com/art.jpg",    url: "https://example.com/art.jpg",    artwork_id: "3213a9dc-ff10-40f2-8a1e-17017d1b40cb",    similarity: 0,    ...v2CleanNegative,
};
