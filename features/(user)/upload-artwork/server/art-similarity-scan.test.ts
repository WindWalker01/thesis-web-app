...
describe("buildSimilarityScanInsert", () => {
  ...
  it("applies the Option-B fallback in the upload flow (zero consensus + strong legacy)", () => {
    const r = result({
      db: match({
        type: "database",
        url: "db-1",
    ...
    // Ranking follows the decision score, so web (80.23) wins.
    const primary = getPrimarySimilarityMatch(r);
    expect(primary?.similarity).toBe(80.23);
    expect(primary?.type).toBe("internet");

    // The report is no longer suppressed: 80.23 clears the render threshold.
    const report = getSimilarityReportMatch(r);
    expect(report?.similarity).toBe(80.23);
    ...
    expect(row.best_similarity_percentage).toBe(80.23);
  });

  it("keeps weak legacy below the gate suppressed in the upload flow", () => {
    const r = result({
      web: match({
        type: "internet",
        url: "web-1",
    ...
    expect(getSimilarityReportMatch(r)).toBeNull();
  });
});
