...
export async function buildRecognitionProfile(
  ...
  const [
    ...
    getBlockchainFact(adminSupabase, input.artId),
    getAuthorshipFact(adminSupabase, input.artId),
  ...
  const sections: RecognitionSection[] = [
      ...
      facts: [
        (() => {
          ...
          if (isPositive) {
            ...
            satisfied = false;
            description =
              "More downvotes than upvotes — mixed community reception";
          } else {
            // Neutral — upvotes === downvotes
            satisfied = up > 0;
            description =
              "Equal upvotes and downvotes — neutral community reception";
          }
    ...
    },
    {
      ...
      facts: [registrationFact, blockchainFact, authorshipFact].filter(
        (f) => f !== null,
      ),
    },
    {
...
async function getAuthorshipFact(
  ...
  const [artworkResult, reviewResult] = await Promise.all([
    adminSupabase
      .from("registered_arts")
      .select("status, tx_hash, chain, work_id")
      .eq("id", artId)
      .maybeSingle(),
    adminSupabase
  ...
  const status = deriveAuthorshipStatus({
    registeredArtStatus: (artwork?.status as RegisteredArtStatus) ?? null,
    hasBlockchainRecord: Boolean(artwork?.tx_hash && artwork?.chain),
    reviewStatus: (review?.status as ArtworkReviewStatus) ?? null,
  });

  const fact = buildAuthorshipFact(status);
...
async function getSimilarityFact(
  ...
  if (!data || data.status !== "completed") {
    return {
      ...
      description:
        "A visual similarity scan has not been performed on this artwork",
    };
  }
