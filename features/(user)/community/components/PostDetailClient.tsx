...
export function PostDetailClient({
  ...
  const { profile: recognitionProfile, isLoading: recognitionLoading } =
    useArtworkRecognitionProfile(post.artId);
  ...
  return (
    <main className="bg-background text-foreground font-display min-h-screen">
      ...
      <div className="mx-auto max-w-7xl px-4 pb-16 pt-24 sm:px-6 lg:px-8">
        <Link
          href="/community"
          className="text-muted-foreground hover:text-foreground mb-6 inline-flex items-center gap-2 text-sm font-medium transition"
        >
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_420px] lg:items-start">
          {/* ── Left Column: Image + Metadata + Actions ── */}
          ...
          {/* ── Right Column: Recognition Profile (sticky) ── */}
          <div className="lg:sticky lg:top-24 lg:self-start space-y-5">
            ...
            {!recognitionProfile && !recognitionLoading && (
              <div className="rounded-3xl border border-border/70 bg-card/85 backdrop-blur-xl p-5">
                <p className="text-sm text-muted-foreground">
                  Artwork recognition data is loading or unavailable.
                </p>
              </div>
            )}
          </div>
