...
export function ArtworkGrid({ artworks, viewMode, sidebarOpen, onClearFilters }: Props) {
    return (
        <div className="flex-1 min-w-0">
            <AnimatePresence mode="wait">
                ...
                {artworks.length === 0 ? (
                    ...
                    >
                        {artworks.map((art, i) => (
                            <ArtworkCardGrid key={art.id} art={art} index={i} />
                        ))}
                    </motion.div>
                ) : (
                    <motion.div
                    ...
                    >
                        {artworks.map((art, i) => (
                            <ArtworkCardList key={art.id} art={art} index={i} />
                        ))}
                    </motion.div>
                )}
