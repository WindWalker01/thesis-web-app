...
export function ArtworkCardGrid({ art, index, scope = "gallery" }: Props) {
  const href =
    scope === "issues"
      ? `/profile/issues/${art.id}`
      : `/profile/artworks/${art.id}`;
  ...
  return (
    <motion.div
      ...
      className="group border-border bg-card hover:border-primary/40 relative overflow-hidden rounded-2xl border transition-all duration-300 hover:shadow-[0_4px_24px_rgba(59,130,246,0.1)]"
    >
      <Link
        href={href}
      ...
      <div className="relative aspect-square overflow-hidden">
        ...
        <div className="absolute top-2 left-2 z-[1]">
          <span className="rounded-full bg-black/50 px-2 py-0.5 text-xs font-bold tracking-widest text-white/90 uppercase backdrop-blur-sm">
            {art.category}
          </span>
        </div>

        {art.hashStatus === "complete" && (
          <div className="absolute top-2 right-12 z-[1]">
      ...
      <div className="p-3">
        ...
        <p className="text-muted-foreground mb-2 text-xs">{art.uploadDate}</p>
