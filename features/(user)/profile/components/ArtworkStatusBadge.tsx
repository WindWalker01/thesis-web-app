...
const statusStyles: Record<ArtworkStatus, string> = {
  ...
  flagged: "bg-red-500/10 border-red-500/20 text-red-600 dark:text-red-400",
  removed:
...
export function ArtworkStatusBadge({ status }: Props) {
  return (
    <span
      className={`inline-flex items-center rounded-full border px-2 py-1 text-xs font-bold tracking-wide uppercase ${statusStyles[status]}`}
    >
