...
const REGISTER_SELECTOR =
  iface.getFunction("registerWorkForUser")?.selector ?? "";
...
async function fetchTransactionsViaRpcLogs(
  ...
  while (toBlock >= 0 && rows.length < neededCount && batches < MAX_BATCHES) {
    ...
    try {
      ...
      if (
        isBlockRangeLimitError(error) &&
        blockBatchSize > MIN_BLOCK_BATCH_SIZE
      ) {
...
async function resolveTransactions(
  ...
  if (POLYGONSCAN_API_KEY) {
    try {
      const explorerData = await fetchTransactionsViaExplorerApi(
        page,
        pageSize,
      );
